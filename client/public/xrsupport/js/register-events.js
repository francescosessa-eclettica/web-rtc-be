  ...
  function sendKey(e, type) {
    ...
    _videoPlayer &&
      _videoPlayer.sendMsg(
        new Uint8Array([InputEvent.Keyboard, type, e.repeat, key, character])
          .buffer
      );
  }
...
export function registerMouseEvents(videoPlayer, playerElement) {
  ...
  sendMouse = (e) => {
    ...
    console.log("send mouse", e.type, x, y);
...
export function sendColor(videoPlayer, hexColor) {
  const r = parseInt(hexColor.slice(1, 3), 16);
  const g = parseInt(hexColor.slice(3, 5), 16);
  const b = parseInt(hexColor.slice(5, 7), 16);

  let data = new DataView(new ArrayBuffer(4));
  data.setUint8(0, InputEvent.ColorPick);
  data.setUint8(1, r);
  data.setUint8(2, g);
  data.setUint8(3, b);
...
export function sendThickness(videoPlayer, thickness) {
  sendInt32Value(videoPlayer, InputEvent.ThicknessSelect, parseInt(thickness));
}

export function sendInt32Value(videoPlayer, inputEvent, int32Value) {
  let data = new DataView(new ArrayBuffer(5));
  data.setUint8(0, inputEvent /*InputEvent.ButtonClick*/);
  data.setInt32(1, int32Value, true);
  videoPlayer && videoPlayer.sendMsg(data.buffer);
}

export function sendText(videoPlayer, text, bufLen) {
  let base64 = btoa(text);

  // alert("base64: " + base64);

  let base64Len = base64.length;
  // let bufLen = text.length * 2;
  let data = new DataView(new ArrayBuffer(base64Len+1));

  data.setUint8(0, InputEvent.Text);

  for (let i = 0; i < base64Len; i++) {
    data.setUint8(i + 1, base64[i].charCodeAt(0));
  }

//  for (let i = base64Len; i < bufLen; i++) {
//    data.setUint8(i, 0);
//  }
